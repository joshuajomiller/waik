...
Claude Code, Codex, and Cursor already fire hooks at the lifecycle moments waik cares about — when a turn starts, when it ends, when it pauses for user input. waik installs into those hook configs and listens on a loopback socket. Activity becomes a direct signal instead of an inferred one.

```
┌─────────────────────────┐  hooks   ┌──────────────────┐
│ Claude / Codex / Cursor │ ───────▶ │ waik-hook (CLI)  │
...
The keep-awake stays on as long as `sessions` is non-empty. Multiple windows of Claude, Codex, and Cursor running concurrently? Each is its own entry; only when the last one closes does the Mac get to sleep.
...
2. **Install the hooks.** Click the menu bar icon → **Agent hooks** → **Install** for `claude`, `codex`, and `cursor`. waik adds entries to:
...
- **`HookInstaller`** owns the JSON/TOML editing. Claude and Cursor entries are tagged with `_waik: true` so uninstall is precise. Codex `notify` gets wrapped by a generated bash script that calls waik-hook *and* forwards to your previous notify program — your Codex desktop notifications keep firing.
...
| Claude hook         | Codex notify type     | Cursor hook          | waik event           | Effect                      |
|---------------------|-----------------------|----------------------|----------------------|------------------------------|
| `UserPromptSubmit`  | —                     | `beforeSubmitPrompt` | `turn_start`         | Add session, engage         |
| `Stop`              | `agent-turn-complete` | `stop`               | `turn_end`           | Remove session              |
...
- waik reads and writes three files in your home directory (`~/.claude/settings.json`, `~/.codex/config.toml`, `~/.cursor/hooks.json`), and only the entries it owns. Uninstall is reversible: backups of the prior Codex `notify` live at `~/Library/Application Support/waik/codex-notify-original.json`.
...
Not yet. Neither exposes a per-turn lifecycle hook waik can install into — Zed has an open feature request for one and VS Code's Copilot Chat doesn't expose this surface at all. The hook-based approach is precise and instant, but limited to tools that fire lifecycle hooks; they land here when those mechanisms ship.
